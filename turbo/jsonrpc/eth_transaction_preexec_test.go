...
// Test strict nonce validation
func TestTransactionPreExec_StrictNonceValidation(t *testing.T) {
	api, bankAddress := setupTestEnvironment(t)
	ctx := context.Background()

	recipient := libcommon.HexToAddress("0x1111111111111111111111111111111111111111")

	t.Run("Sequential nonces should work", func(t *testing.T) {
		// Create batch with sequential nonces
		transactions := []PreArgs{
			createTestPreArgs(bankAddress, recipient, 0, nil, big.NewInt(1e15)),
			createTestPreArgs(bankAddress, recipient, 1, nil, big.NewInt(1e15)),
		...
		for i, result := range results {
			assert.Empty(t, result.Error.Msg, "Transaction %d should succeed", i)
			assert.Greater(t, result.GasUsed, uint64(0), "Transaction %d should use gas", i)
			t.Logf("✅ Transaction %d: GasUsed=%d", i, result.GasUsed)
		}
	})

	t.Run("Gap in nonces should fail", func(t *testing.T) {
		// Create batch with gap in nonces (0, 2 - missing 1)
		transactions := []PreArgs{
			createTestPreArgs(bankAddress, recipient, 0, nil, big.NewInt(1e15)),
			createTestPreArgs(bankAddress, recipient, 2, nil, big.NewInt(1e15)), // Gap: should be 1
		}
		...
		assert.Greater(t, results[0].GasUsed, uint64(0), "First transaction should use gas")

		// Second transaction should fail due to nonce gap
		assert.NotEmpty(t, results[1].Error.Msg, "Second transaction should fail")
		assert.Contains(t, results[1].Error.Msg, "nonce mismatch", "Should mention nonce mismatch")
		assert.Contains(t, results[1].Error.Msg, "expected 1", "Should expect nonce 1")

		t.Logf("✅ Expected failure: %s", results[1].Error.Msg)
	})

	t.Run("Starting with wrong nonce should fail", func(t *testing.T) {
		// Create batch starting with wrong nonce
		transactions := []PreArgs{
			createTestPreArgs(bankAddress, recipient, 5, nil, big.NewInt(1e15)), // Should start with 0
		}
		...
		assert.NotEmpty(t, results[0].Error.Msg, "Transaction should fail")
		assert.Contains(t, results[0].Error.Msg, "nonce mismatch", "Should mention nonce mismatch")
		assert.Contains(t, results[0].Error.Msg, "expected 0", "Should expect nonce 0")

		t.Logf("✅ Expected failure: %s", results[0].Error.Msg)
	})
}
