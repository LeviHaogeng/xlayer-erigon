...
type txPool interface {
	...
	PeekBest(n uint16, txs *types.TxsRlp, tx kv.Tx, onTopOf, nextBlockNumber, availableGas, availableBlobGas uint64) (bool, error)
...
func (s *GrpcServer) Pending(ctx context.Context, _ *emptypb.Empty) (*txpool_proto.PendingReply, error) {
	...
	if _, err := s.txPool.PeekBest(math.MaxInt16, &txSlots, tx, 0 /* onTopOf */, 0, math.MaxUint64 /* available gas */, 0); err != nil {
		return nil, err
	}
