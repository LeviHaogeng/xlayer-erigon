...
// ExpiredFilterCleaner defines the interface for cleaning up expired filters
type ExpiredFilterCleaner interface {
	CleanupExpired(expirationTime int64) int
}

// FilterCleaner manages TTL-based cleanup for a specific type of filters
type FilterCleaner[T comparable] struct {
	name            string
	cleanupFunc     func(T) bool
	logger          log.Logger
	lastAccessTimes *concurrent.SyncMap[T, int64]
}

// NewFilterCleaner creates a new FilterCleaner instance
func NewFilterCleaner[T comparable](
	name string,
	cleanupFunc func(T) bool,
	logger log.Logger,
) *FilterCleaner[T] {
	return &FilterCleaner[T]{
		name:            name,
		cleanupFunc:     cleanupFunc,
		logger:          logger,
		lastAccessTimes: concurrent.NewSyncMap[T, int64](),
	}
}

// CleanupExpired removes filters that have been inactive longer than the expiration threshold
// Returns the number of filters successfully cleaned up
func (c *FilterCleaner[T]) CleanupExpired(expirationTime int64) int {
	var expiredIDs []T

	// Collect expired filter IDs first to avoid modifying map during iteration
	c.lastAccessTimes.Range(func(id T, lastTime int64) error {
	...
	log.Info(fmt.Sprintf("TTL reaper: cleanupExpired: expiredIDs: %v", expiredIDs))

	for _, id := range expiredIDs {
		c.lastAccessTimes.Delete(id)
		c.cleanupFunc(id)
	}
...
// UpdateLastAccessTime updates the last access time for the given filter ID to the current timestamp.
// This is used to track when a filter was last accessed for TTL-based cleanup.
func (c *FilterCleaner[T]) UpdateLastAccessTime(id T) {
	c.lastAccessTimes.Put(id, time.Now().Unix())
}

// Delete deletes the last access time for the given filter ID.
func (c *FilterCleaner[T]) Delete(id T) {
	c.lastAccessTimes.Delete(id)
}
...
type Filters struct {
	...
	// TTL cleaners for different filter types
	logsTTLCleaner         *FilterCleaner[LogsSubID]
	pendingHeadsTTLCleaner *FilterCleaner[HeadsSubID]
	pendingTxsTTLCleaner   *FilterCleaner[PendingTxsSubID]
}
...
func New(ctx context.Context, config FiltersConfig, ethBackend ApiBackend, txPool txpool.TxpoolClient, mining txpool.MiningClient, onNewSnapshot func(), logger log.Logger) *Filters {
	...
	// Initialize TTL cleaners for different filter types
	ff.logsTTLCleaner = NewFilterCleaner[LogsSubID]("logs", ff.UnsubscribeLogs, logger)
	ff.pendingHeadsTTLCleaner = NewFilterCleaner[HeadsSubID]("heads", ff.UnsubscribeHeads, logger)
	ff.pendingTxsTTLCleaner = NewFilterCleaner[PendingTxsSubID]("txs", ff.UnsubscribePendingTxs, logger)

	// start background TTL cleaner if configured
	if config.RpcSubscriptionFiltersTTLSeconds > 0 && config.RpcSubscriptionFiltersCleanupIntervalSeconds > 0 {
		interval := time.Duration(config.RpcSubscriptionFiltersCleanupIntervalSeconds) * time.Second
		go ff.runTTLReaper(ctx, interval, time.Duration(config.RpcSubscriptionFiltersTTLSeconds)*time.Second)
	}
		...
		go func() {
			...
			for {
				...
				if err := ff.subscribeToPendingTransactions(ctx, txPool); err != nil {
					...
					if grpcutil.IsEndOfStream(err) || grpcutil.IsRetryLater(err) || isTxPoolDisabledErr(err) {
						time.Sleep(3 * time.Second)
						continue
					}
...
func (ff *Filters) SubscribeNewHeads(size int) (<-chan *types.Header, HeadsSubID) {
	return ff.subscribeNewHeads(size, false)
}

// SubscribeNewHeadsWithTTL subscribes to new block headers with TTL tracking for automatic cleanup.
func (ff *Filters) SubscribeNewHeadsWithTTL(size int) (<-chan *types.Header, HeadsSubID) {
	return ff.subscribeNewHeads(size, true)
}

// subscribeNewHeads is the internal implementation that optionally enables TTL tracking.
func (ff *Filters) subscribeNewHeads(size int, enableTTL bool) (<-chan *types.Header, HeadsSubID) {
	id := HeadsSubID(generateSubscriptionID())
	sub := newChanSub[*types.Header](size)
	ff.headsSubs.Put(id, sub)

	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "heads"}).Inc()

	// Set initial lastAccess time for TTL tracking (only for polling filters)
	if enableTTL && ff.config.RpcSubscriptionFiltersTTLSeconds > 0 {
		ff.pendingHeadsTTLCleaner.UpdateLastAccessTime(id)
	}
...
func (ff *Filters) UnsubscribeHeads(id HeadsSubID) bool {
	...
	ff.pendingHeadsTTLCleaner.Delete(id)

	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "heads"}).Dec()
...
func (ff *Filters) SubscribePendingLogs(size int) (<-chan types.Logs, PendingLogsSubID) {
	...
	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_logs"}).Inc()
...
func (ff *Filters) UnsubscribePendingLogs(id PendingLogsSubID) {
	...
	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_logs"}).Dec()
}
...
func (ff *Filters) SubscribePendingBlock(size int) (<-chan *types.Block, PendingBlockSubID) {
	...
	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_block"}).Inc()
...
func (ff *Filters) UnsubscribePendingBlock(id PendingBlockSubID) {
	...
	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_block"}).Dec()
}
...
func (ff *Filters) SubscribePendingTxs(size int) (<-chan []types.Transaction, PendingTxsSubID) {
	return ff.subscribePendingTxs(size, false)
}

// SubscribePendingTxsWithTTL subscribes to pending transactions with TTL tracking for automatic cleanup.
func (ff *Filters) SubscribePendingTxsWithTTL(size int) (<-chan []types.Transaction, PendingTxsSubID) {
	return ff.subscribePendingTxs(size, true)
}

// subscribePendingTxs is the internal implementation that optionally enables TTL tracking.
func (ff *Filters) subscribePendingTxs(size int, enableTTL bool) (<-chan []types.Transaction, PendingTxsSubID) {
	id := PendingTxsSubID(generateSubscriptionID())
	sub := newChanSub[[]types.Transaction](size)
	ff.pendingTxsSubs.Put(id, sub)

	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_txs"}).Inc()

	// Set initial lastAccess time for TTL tracking (only for polling filters)
	if enableTTL && ff.config.RpcSubscriptionFiltersTTLSeconds > 0 {
		ff.pendingTxsTTLCleaner.UpdateLastAccessTime(id)
	}
...
func (ff *Filters) UnsubscribePendingTxs(id PendingTxsSubID) bool {
	...
	ff.pendingTxsTTLCleaner.Delete(id)

	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "pending_txs"}).Dec()
...
func (ff *Filters) SubscribeLogs(size int, criteria filters.FilterCriteria) (<-chan *types.Log, LogsSubID) {
	return ff.subscribeLogs(size, criteria, false)
}

// SubscribeLogsWithTTL subscribes to logs with TTL tracking for automatic cleanup.
func (ff *Filters) SubscribeLogsWithTTL(size int, criteria filters.FilterCriteria) (<-chan *types.Log, LogsSubID) {
	return ff.subscribeLogs(size, criteria, true)
}

// subscribeLogs is the internal implementation that optionally enables TTL tracking.
func (ff *Filters) subscribeLogs(size int, criteria filters.FilterCriteria, enableTTL bool) (<-chan *types.Log, LogsSubID) {
	...
	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "logs"}).Inc()

	// Set initial lastAccess time for TTL tracking (only for polling filters)
	if enableTTL && ff.config.RpcSubscriptionFiltersTTLSeconds > 0 {
		ff.logsTTLCleaner.UpdateLastAccessTime(id)
	}
...
func (ff *Filters) UnsubscribeLogs(id LogsSubID) bool {
	...
	ff.logsTTLCleaner.Delete(id)

	activeSubscriptionsGauge.With(prometheus.Labels{filterLabelName: "logs"}).Dec()
...
// runTTLReaper periodically evicts polling filter stores that have been inactive longer than ttl.
func (ff *Filters) runTTLReaper(ctx context.Context, interval time.Duration, ttl time.Duration) {
	ticker := time.NewTicker(interval)
	defer ticker.Stop()

	ttlCleaners := []ExpiredFilterCleaner{
		ff.logsTTLCleaner,
		ff.pendingHeadsTTLCleaner,
		ff.pendingTxsTTLCleaner,
	}
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			expirationTime := time.Now().Unix() - int64(ttl/time.Second)
			for _, cleaner := range ttlCleaners {
				cleaner.CleanupExpired(expirationTime)
			}
		}
	}
}
